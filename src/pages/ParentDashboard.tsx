    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { icon: Clock, label: 'Tempo Total', value: `${state.totalPlayTime}min`, color: 'blue' },
            { icon: BookOpen, label: 'Lições', value: `${state.completedLessons.length}/${lessons.length}`, color: 'green' },
            { icon: TrendingUp, label: 'Nível', value: state.level, color: 'purple' },
            { icon: Award, label: 'Medalhas', value: state.medals.length, color: 'yellow' },
          ].map((s, i) => {
            const Icon = s.icon
            return (
            ...
            {weeklyData.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                ...
                <motion.div
                  ...
                  className="w-full rounded-t-lg bg-gradient-to-t from-purple-600 to-purple-400 min-h-[4px]"
                />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-md">
            <h2 className="text-lg font-bold mb-4">Habilidades Desenvolvidas</h2>
            <div className="space-y-3">
              {[
                ...
                { name: 'Concentração', pct: Math.min(completionPct, 100) },
